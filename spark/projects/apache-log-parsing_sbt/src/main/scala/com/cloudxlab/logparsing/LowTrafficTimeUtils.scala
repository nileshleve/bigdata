...
class LowTrafficTimeUtils extends Serializable {
    ...
    def gettop5(accessLogs:RDD[String], sc:SparkContext, bottomn:Int):Array[(String,Int)] = {
        ...
        var sortedfrequencies = frequencies.sortBy(x => x._2, true)
        return sortedfrequencies.take(bottomn)
    }
}
